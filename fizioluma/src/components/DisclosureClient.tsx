      ...
      {({ open }) => (
        <div className="flex flex-wrap items-center justify-between w-full lg:w-auto">
          ...
          <DisclosureButton
            ...
            className="px-2 py-1 ml-auto text-gray-500 rounded-md lg:hidden hover:text-themecolor focus:text-themecolor focus:bg-indigo-100 focus:outline-none "
          >
            <svg
            ...
            <>
              {navigation.map((item, index) => (
                <Link
                  key={index}
                  href={item.href}
                  className="w-full px-4 py-2 -ml-4 text-gray-500 rounded-md :text-gray-300 hover:text-themecolor focus:text-themecolor focus:bg-indigo-100 focus:outline-none"
                >
                  {item.text}
                </Link>
              ))}
